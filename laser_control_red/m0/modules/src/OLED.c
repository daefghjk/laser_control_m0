...
//length1：整数部分个数       length2：小数部分个数
void OLED_ShowFloat(uint8_t Line, uint8_t Column, float Num, uint8_t Length1, uint8_t Length2)
{
    if (Num < 0)
    {
        OLED_ShowChar(Line, Column, '-');
    ...
    OLED_ShowNum(Line, Column + 1, (int)Num, Length1);
    OLED_ShowChar(Line, Column + Length1 + 1, '.');
    OLED_ShowNum(Line, Column + Length1 + 2, (int)(Num * OLED_Pow(10, Length2)) % OLED_Pow(10, Length2), Length2);
}
